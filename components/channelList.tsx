...
export default function ChannelList() {
    ...
    const columns = useMemo(
        () => [
            {
                Header: 'Msg',
                accessor: (d) => (<span style={{cursor: 'pointer', fontSize: 20}} onClick={() => handleCompose(d.id)}>✍️</span>)
            },
            {
                Header: 'ID',
                accessor: (d) => (<span style={{cursor: 'pointer', textDecoration: 'underline'}} onClick={() => handleClick(d.id)}>{d['id']}</span>)
            },
            {
                Header: 'Name',
                accessor: 'name'
